...
<head>
	...
	<link rel="stylesheet" href="stylede.css?v=<?=rand(1,324234234)?>">
...
<div class = "s1">	 
	...
	<p class= "invate-text">Wir freuen uns dir mitteilen zu können, dass das wichtigste Fest unseres Lebens, unsere Hochzeit, am <span class = "words">17.09.2022</span> stattfinden wird!<br><br>

	Wir laden dich herzlich ein, die Freude dieses unvergesslichen Tages mit uns zu teilen. Am <span class = "words">17.09.2022</span> erwarten wir dich in der sonnigen Türkei!</p>
	
	<picture>
		<source srcset="images/foto2mb.jpg"  class="foto2" media="(max-width: 580px)">
	    <img src="images/foto2dt.jpg" alt="" class="foto2">
	</picture>

	<p class = "timer-wait">Wir zählen jeden einzelnen Tag bis zum Beginn unserer Hochzeit runter</p>	
    ...
    <p class = "text"> Uns ist es sehr wichtig, Antworten bis zum 30.11.2021 auf die folgenden Fragen zu bekommen...</p>
</div>
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST') {
		  ...
		  /*  if (empty($_POST["firstname"]) || empty($_POST["lastname"])) {
		  	...
		  	if (empty($_POST["choice"])) {
		    		$errors[1] = "Eine Frage wurde nicht beantwortet...";
		  	} 
		  	...
		  	if (empty($_POST["food"])) {
		    		$errors[3] = "Welches Essen bevorzugst du?";
		  	}  

		  	if (empty($_POST["drink"])) {
		    		$errors[4] = "Welche Getränke bevorzugst du?";
		  	}   

		  	if (empty($_POST["pfood"])) {
		    		$errors[5] = "Was deine Hochzeitsbegleitung beim Essen bevorzugt...";
		  	}  

		  	if (empty($_POST["pdrink"])) {
		    		$errors[6] = "Was deine Hochzeitsbegleitung bei den Getränken bevorzugt...";
		  	}   
		  	...
		  	if (isset($_POST["choice"])) {
				...
				$count = $result->num_rows; 
			  	...
			  	if($_POST["choice"] == "Не смогу прийти") {	
					...
					$errors = array();
					echo "<span class = 'thanks'>"."Sehr schade, dass du nicht anwesend sein kannst"."</span>";
					$mysqli -> close();
					...
					/*Если партнер есть, то обновляем, если нет - добавляем */
						if($count > 0) {
							...
							echo "<span class = 'thanks'>"."Herzlichen Dank für deine Angaben!"."</span>";
						} 
						else {
			  		...
			  		echo "<span class = 'check'>"."Wahrscheinlich hast du eine Frage übersehen"."</span>";
		  		}
		  	}	
		} 	
?>
	...
	<h4>Wirst du zu unserer Hochzeit kommen?</h4>
		<div>
			<label class="form__choice"><input type="radio"  class="form__decision" name="choice" value="Я прийду" >Ja, ich werde kommen</label><br>	
			<label  class="form__choice"><input type="radio" class="form__decision" name="choice" value="Скорее всего буду">Ja, ich werde sehr wahrscheinlich kommen</label><br>
			<label class="form__choice"><input type="radio" class="form__decision" name="choice" value="Не смогу прийти">Nein, ich kann leider nicht kommen</label><br>
		</div>
	</div>
	...
	<?php
		...
		echo "<span class = 'errors'>".$errors[4]."</span>";
					...
					<h4>Bei den Getränken bevorzuge ich...</h4>
					<label class="form__choice"><input type="checkbox" name="drink[0]" value="1">Bier</label><br>
					<label class="form__choice"><input type="checkbox" name="drink[1]" value="1">Weißwein</label><br>
					<label class="form__choice"><input type="checkbox" name="drink[2]" value="1">Rotwein</label><br>
	...
	<?php
		...
		echo "<span class = 'errors'>".$errors[3]."</span>";
					...
					<h4>Beim Essen bevorzuge ich...</h4>
					<label class="form__choice"><input type="checkbox" name="food[0]" value="1">Gerichte mit Fleisch</label><br>
					<label class="form__choice"><input type="checkbox" name="food[1]" value="1">Gerichte mit Fisch</label><br>
					<label class="form__choice"><input type="checkbox" name="food[2]" value="1">Gerichte mit Meeresfrüchten</label><br>
					<label class="form__choice"><input type="checkbox" name="food[3]" value="1">vegetarische Gerichte</label><br>

					<textarea class= "food_comment" placeholder="Wenn du Allergien, spezielle Essens- oder Getränkewünsche hast, dann teile uns dies hier bitte mit." name ="food-comment"></textarea>
					</div>
	<?php
		if(isset($errors["2"])) {
		echo "<span class = 'errors'>".$errors[2]."</span>";
				...
				<label class="form__choice"><input type="radio" class="form__choice_partner" name="partner" value="2">mit meiner Ehefrau/Freundin</label><br>
					...
					<div class= "partner_preferences" style="display: none;"> 
							...
							?>
							<hr>
							<h4>Meine Hochzeitsbegleitung trinkt...</h4>
							...
							?>
							<h4>Beim Essen bevorzugt meine Hochzeitsbegleitung...</h4>
							<label class="form__choice"><input type="checkbox" name="pfood[0]" value="1">Gerichte mit Fleisch</label><br>
							<label class="form__choice"><input type="checkbox" name="pfood[1]" value="1">Gerichte mit Fisch</label><br>
							<label class="form__choice"><input type="checkbox" name="pfood[2]" value="1">Gerichte mit Meeresfrüchten</label><br>
							<label class="form__choice" id = pfood3><input type="checkbox" name="pfood[3]" value="1"><span></span></label><br>

							<textarea class= "food_comment" placeholder="Wenn deine Hochzeitsbegleitung Allergien, spezielle Essens- oder Getränkewünsche hat, dann teile uns dies hier bitte mit." name ="pcommentFood"></textarea>
							<hr>
					</div>

				<h4><label><input type="checkbox" class="form__children" name="children" value="Да">Mit mir kommen auch meine Kinder mit:</label></h4><br>
			
			
				<div class = "wrapper-form-children">
					<h6 class = "age">von 0 bis 5 Jahre</h6>
					<input type="text" class="form__name" name="child-baby" placeholder="Gib hier die Anzahl deiner Kinder ein"><br>
					<h6 class = "age">von 6 bis 13 Jahre</h6>
					<input type="text" class="form__name" name="child-medium" placeholder="Gib hier die Anzahl deiner Kinder ein"><br>
					<h6 class = "age">von 14 Jahre und älter </h6>
					<input type="text" class="form__name" name="child-teenager" placeholder="Gib hier die Anzahl deiner Kinder ein"><br>
				</div>	
			</div>
...
<div class = "s2">
	<div class = "wedding-style"> 
		<p class = "wedding-style__text">Wir werden uns sehr freuen, wenn dein Hochzeitsoutfit den folgenden Farben ensprechen wird</p>
		<div class = "colors"> 
			...
			<div class = "color" style = "background-color: #f1cbcb;"></div> 
			<div class = "color" style = "background-color: #eee2d6;"></div>
		</div>
	</div>
	<hr class = "line">
	<p class = "text-information">Nach der Hochzeit findest du unter diesem Link Videos und Fotos von unserer Hochzeit. Diese stehen dir dann zum Download zur Verfügung.</p>
