...
for (let i = 0; i< choicePartner.length; i++) {
	choicePartner[i].addEventListener('change', function() {
		if (choicePartner[1].checked) {
			...
			pfood3.getElementsByTagName('span')[0].innerHTML = "he is vegetarian";
		}else if( choicePartner[2].checked) {
			guestPreferences.style.display = "none";
			partnerPreferences.style.display = "block";
			pfood3.getElementsByTagName('span')[0].innerHTML = "she is vegetarian";
		}else {
...
function hideForm() {
    if(thanks && thanks.textContent == "It is a pity, that you can't come") {
    ...
    } else if (thanks && thanks.textContent == 'Thank you for your answer!') {
		mainQuestion.style.display = 'none';
		button.style.display = 'none';
		section1.style.display = 'none';
